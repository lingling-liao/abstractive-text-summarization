...
# Maxlen: taking values > and round figured to 75th percentile, at the same time
# not leaving high variance
encoder_maxlen = 400
decoder_maxlen = 75
...
def positional_encoding(position, d_model):
    """Positional Encoding for adding notion of position among words as unlike
    RNN this is non-directional.
    """
...
def create_look_ahead_mask(size):
    """Lookahead mask for masking future words from contributing in prediction
    of current words in self attention.
    """
...
class Transformer(tf.keras.Model):
    ...
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
        ...
        self.decoder = Decoder(
            num_layers, d_model, num_heads, dff, target_vocab_size, pe_target,
            rate)
...
def summarize(input_document):
    # Not considering attention weights for now, can be used to plot attention
    # heatmaps in the future
    summarized = evaluate(input_document=input_document)[0].numpy()
    # not printing <go> token
